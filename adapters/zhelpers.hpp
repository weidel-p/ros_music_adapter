...
enum STR2INT_ERROR { SUCCESS_, OVERFLOW_, UNDERFLOW_, INCONVERTIBLE_ };
...
static int 
s_recvAsInt(zmq::socket_t & socket)
{
    std::string message = s_recv(socket);
    std::vector<std::string> data = split(message, ' ');
    //return data[1];
    int ret;
    if (str2int(ret, data[1]) == SUCCESS_)
        return ret;
}

void
s_recvAsVector(zmq::socket_t & socket, double* data)
{
    std::string message = s_recv(socket);
