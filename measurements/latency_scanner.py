...
def create_music_config_neuron(num_neurons, sim_time, timestep):
    music_config = \
                "stoptime=" + str(sim_time) + "\n"\
                "[sensor]\n\
                  binary=../ros_sensor_adapter\n\
    ...
    with open("NEURON_measurement_params.dat", "w") as f:
        json.dump({"t": sim_time, "n": num_neurons, "ts": timestep, "s": 1}, f)
...
for timestep in np.arange(MIN_TIMESTEP, MAX_TIMESTEP, TIMESTEP_STEP_SIZE):
    ...
    create_music_config_neuron(1, sim_time, timestep)
    ...
    for it in range(ITERATIONS):
        ...
        insert_datapoint(it, latency, timestep, "with NEURON")
